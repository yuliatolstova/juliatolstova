...
export const ConnectPopup = ({ connectPopupOpen, setConnectPopupOpen }) => {
    ...
    return (<div className={`${CN}__background ${connectPopupOpen ? 'show' : ''}`}
                 onClick={handleClose}>
        <div className={`${CN}__content`}>
            <aside>
                <h3 className={`${CN}__title`}>Связаться со мной</h3>
                <button className={`${CN}__close`} onClick={() => {
                    setConnectPopupOpen(false);
                }} />
                <p>Если вы хотите задать вопрос, пожалуйста, свяжитесь со мной, и я отвечу как можно скорее. Если вы
                    предпочитаете связаться со мной напрямую, отправьте электронное письмо по адресу: <a
                        href={'mailto:tolstova.dev@gmail.com'}>tolstova.dev@gmail.com</a></p>
            </aside>
            ...
            <div className={`${CN}__body`}>
                ...
                <button className={`${CN}__send`} onClick={handleSend} >Отправить</button>
            </div>
        </div>
    </div>)
        ;
